    ...
    <div className="flex flex-col gap-3.5">
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSave)} className="flex flex-col gap-3.5 max-w-[48rem]">
          <FormField name="statsDisabled" control={form.control} render={({ field }) => (
            <FormItem className='flex flex-row items-center gap-1.5 pt-2'>
              <FormControl><Checkbox id="disableStats" checked={field.value} onCheckedChange={field.onChange} /></FormControl>
          ...
          <div className='flex flex-row gap-2.5'>
            ...
            <Button disabled={!form.formState.isDirty} onClick={resetForm} type="button" variant={"outline"}>Reset</Button>
          </div>
        </form>
      </Form>
    </div>
